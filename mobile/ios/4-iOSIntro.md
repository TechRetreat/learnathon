...
1. App startup
  - Remember how our application starts in "AppDelegate.swift"? Let's take a look where exactly that happens.
  - Open the "AppDelegate.swift" file and find the `func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool` function.
    - Although it has a long name it's job is very simple. It is called when the program starts, then it returns `true` if everything to get the app setup goes alright.
    ...
    - Next let's make a function that looks like this:
      ...
      ```
      - This function [instantiates]() our menu [view controller]()
...
2. View Controllers
  - "[Views]()" are elements on the screen that users interact with (e.g. Buttons, cells, labels, etc...)
  ...
  - Let's start by adding a label. We will add it in the [`viewDidLoad`]() method
    ...
    - Now that `helloLabel` is a [property]() of the [view controller]() we can go ahead and access it in [`viewDidLoad`]() with `self.helloLabel`.
      - Then we want to set the `text`, `font`, and `textColor` properties of the label. For the font, you can set it like `self.helloLabel.font = UIFont.systemFontOfSize(20)`. Give the other two properties a shot! (hint: colours are [object]()s too! You can create the colour red [object]() using `UIColor.redColor()`.
...
4. Tidying up.
  - If you haven't already, make sure to commit your code ("Source Control" -> "Commit")
